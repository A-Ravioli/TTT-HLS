"""NanoCoder v3: hidden=512 (~28M, MiniMind-scale ReLU) + BPE-8k, to clear the coherence bar.
...
app = modal.App("nanocoder-v3-train")
...
vol = modal.Volume.from_name("nanocoder-v3-ckpt", create_if_missing=True)


@app.function(image=image, gpu="A100", timeout=60 * 60 * 2,
              volumes={"/ckpt": vol}, secrets=[modal.Secret.from_name("nanocoder-hf")])
def train(steps: int = 20000, batch: int = 96, seq: int = 256, lr: float = 4e-4,
          warmup: int = 600, vocab_size: int = 8192, hidden: int = 512, inter: int = 2048,
          layers: int = 8, heads: int = 16, max_rows: int = 600000, cap_mb: int = 400) -> dict:
    import os
    import time
