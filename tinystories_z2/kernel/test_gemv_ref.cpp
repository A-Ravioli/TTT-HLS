// test_gemv_ref.cpp -- verify the C++ W8A8 GEMV datapath against Python golden.
//
// Reads <golden_dir>/cases.tsv and, for each case, loads the INT8 weights, fp16
// scales, INT8 activation and expected fp32 output produced by
// tinystories_z2/reference/make_golden.py, runs gemv_int8_compute(), and reports
// max abs/rel error. Exit non-zero if any case exceeds the tolerance.
//
// Bit-level proof that the kernel math matches the quantized TinyStories
// reference *before* any HLS synthesis -- Milestone A, every projection shape.
#include <cstdio>
#include <cstdlib>
#include <cstring>
#include <cmath>
...
int main(int argc, char** argv) {
    ...
    while (std::getline(tsv, line)) {
        ...
        char name[256]; int M, N; double xscale;
        if (sscanf(line.c_str(), "%255[^\t]\t%d\t%d\t%lf",
                   name, &M, &N, &xscale) != 4) continue;

        std::string base = dir + "/" + name;
        auto W  = read_bytes(base + ".W.int8.bin");
        auto S  = read_bytes(base + ".W.scale.bin");
        auto X  = read_bytes(base + ".x.int8.bin");
        auto Yg = read_bytes(base + ".y.f32.bin");

        const int8_t* w = reinterpret_cast<const int8_t*>(W.data());
        ...
        gemv_int8_compute(w, scales, x, (float)xscale, y.data(), M, N);
        ...
        if (!ok) {
            ++n_fail;
            printf("  FAIL %-28s M=%-6d N=%-6d max_abs=%.3e rel=%.3e\n",
                   name, M, N, max_abs, rel);
        }
        if (++n_cases <= 8 || !ok)
            printf("  %-4s %-26s M=%-6d N=%-6d rel=%.3e\n",
